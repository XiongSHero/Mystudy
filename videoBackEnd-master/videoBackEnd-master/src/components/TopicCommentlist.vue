    ...
    <el-table-column label="操作">
      <template scope="scope">
         <el-button
          ...
          @click="banComment(scope.row.id,scope.row)">小黑屋</el-button>
          <el-button
          size="small"
          type="info"
...
import { LOCALHOST_URL } from '../assets/js/localhost.js'
  ...
  export default {
    ...
    created () {
        this.$store.dispatch('setTitlename', {name:'话题评论管理'})
        this.$http.get(''+LOCALHOST_URL+'/api/getTopicComment').then((response)=>{
    ...
    methods: {
      ...
      currentPageNum(val) {
        ...
        this.$http.post(''+LOCALHOST_URL+'/api/getTopicCommentPage',{
          pageNum: getpageNum
        }).then((response)=>{
          let body = response.body;
          var data = [];
      ...
      DeleteComment(id, index, rows) {
        this.$confirm('此操作将删除该分类及其所属该分类的所有内容, 请慎重!', '提示', {
          ...
          type: 'warning'
            ...
            this.$http.post(''+LOCALHOST_URL+'/api/topicCommentDelete',{
            id : id
          }).then((response) => {
             console.log('删除成功')
            },(response)=>{

...
// 测试
router.post('/api/get',function (req, res) {
  console.log(req);
})
...
router.get('/api/getCategorymovie',function(req, res){
    var id = req.query.id;
    connection.query("SELECT movie.id,movie.Mname,movie.PicUrl,movie.showTime,movie.movieUrl,movie.moviePlayTime,movie.enable, movieCategory.name FROM movie LEFT OUTER JOIN movieCategory ON movie.fid = movieCategory.fid  WHERE movie.fid='"+id+"' AND movie.enable=1  order by movie.id LIMIT 4",function(err,doc){
        if (err) {
          console.log(err)
        } else if (doc) {
          res.json({
...
router.post('/api/getCategorymoviePage', function(req, res){
    ...
    var limitNum = '4';
    var id = req.body.id;
      connection.query('SELECT movie.id,movie.Mname,movie.PicUrl,movie.showTime,movie.movieUrl,movie.moviePlayTime,movie.enable, movieCategory.name FROM movie LEFT OUTER JOIN movieCategory ON movie.fid = movieCategory.fid  WHERE movie.fid='+id+' AND movie.enable=1 order by movie.id limit '+startPage+','+limitNum+' ',function(err,doc){
        if(err){
           res.send(err);
        }else if (doc) {
           res.json(doc);
